...
excerpt: "한국어 FAQ 챗봇 LangSmith에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LangChain & LangSmi... 순서로 핵심 장면을 먼저 훑고, Step 2: FAQ 데이터 준비, 4.3 RAG 체인 구성, Step 6: 대화 기록 기..."
research_summary: "한국어 FAQ 챗봇 LangSmith에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LangChain & LangSmi... 순서로 핵심 장면을 먼저 훑고, Step 2: FAQ 데이터 준비, 4.3 RAG 체인 구성, Step 6: 대화 기록 기능 추가 (... 같은 코드로 실제 구현을 이어서 확인할 수 있습니다. `ipynb/md` 원본과 17개 코드 블록, 15개 실행 셀을 함께 남겨 구현 흐름을 다시 따라갈 수 있게 정리했습니다. 주요 스택은 os, getpass, langchain_openai, langchain_core입니다."
...
한국어 FAQ 챗봇 LangSmith에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LangChain & LangSmi... 순서로 핵심 장면을 먼저 훑고, Step 2: FAQ 데이터 준비, 4.3 RAG 체인 구성, Step 6: 대화 기록 기능 추가 (... 같은 코드로 실제 구현을 이어서 확인할 수 있습니다. `ipynb/md` 원본과 17개 코드 블록, 15개 실행 셀을 함께 남겨 구현 흐름을 다시 따라갈 수 있게 정리했습니다. 주요 스택은 os, getpass, langchain_openai, langchain_core입니다.

**빠르게 볼 수 있는 포인트**: LangChain & LangSmith 실습: 한국어 FAQ 챗봇 만들기.
...
Step 1: 환경 설정, Step 6: 대화 기록 기능 추가..., Step 8: 실전 배포를 위한 추... 같은 코드를 직접 따라가며 LangChain & LangSmith 실습: 한국어 FAQ 챗봇 만들기 흐름을 확인했습니다.

- 읽을 포인트: 세부 흐름: Step 1: 환경 설정, Step 6: 대화 기록 기능 추가 (Memory), Step 8: 실전 배포를 위한 추가 기능

#### Step 1: 환경 설정

LangChain & LangSmith 실습: 한국어 FAQ 챗봇 만들기 > Step 1: 환경 설정 아래 코드와 함께 읽으면 구현 포인트가 더 또렷해지는 구간입니다.

#### Step 6: 대화 기록 기능 추가 (Memory)

LangChain & LangSmith 실습: 한국어 FAQ 챗봇 만들기 > Step 6: 대화 기록 기능 추가 (Memory) 아래 코드와 함께 읽으면 구현 포인트가 더 또렷해지는 구간입니다.

#### Step 8: 실전 배포를 위한 추가 기능

LangChain & LangSmith 실습: 한국어 FAQ 챗봇 만들기 > Step 8: 실전 배포를 위한 추가 기능 아래 코드와 함께 읽으면 구현 포인트가 더 또렷해지는 구간입니다.
...
1. LangChain & LangSmith 실습: 한국어 FAQ 챗봇 만들기: Step 1: 환경 설정, Step 6: 대화 기록 기능 추가 (Memory)
...
`4.3 RAG 체인 구성`는 이 노트에서 핵심 구현을 보여주는 코드 블록입니다. 코드 안에서는 최신 LangChain 스타일(2025), RAG 프롬프트 템플릿, ChatPromptTemplate 사용 흐름이 주석과 함께 드러납니다.
