...
excerpt: "LangChain활용에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LangChain을 활용한 RAG 실습 순서로 핵심 장면을 먼저 훑고, LangChain 프롬프트 엔지니어링, LLM - Prompt 체인 만들기 (..., 롤 토큰 Chat 템플..."
research_summary: "LangChain활용에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LangChain을 활용한 RAG 실습 순서로 핵심 장면을 먼저 훑고, LangChain 프롬프트 엔지니어링, LLM - Prompt 체인 만들기 (..., 롤 토큰 Chat 템플릿 같은 코드로 실제 구현을 이어서 확인할 수 있습니다. `ipynb/md` 원본과 10개 코드 블록, 6개 실행 셀을 함께 남겨 구현 흐름을 다시 따라갈 수 있게 정리했습니다. 주요 스택은 getpass, langchain_openai, os, langchain_core입니다."
...
LangChain활용에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LangChain을 활용한 RAG 실습 순서로 핵심 장면을 먼저 훑고, LangChain 프롬프트 엔지니어링, LLM - Prompt 체인 만들기 (..., 롤 토큰 Chat 템플릿 같은 코드로 실제 구현을 이어서 확인할 수 있습니다. `ipynb/md` 원본과 10개 코드 블록, 6개 실행 셀을 함께 남겨 구현 흐름을 다시 따라갈 수 있게 정리했습니다. 주요 스택은 getpass, langchain_openai, os, langchain_core입니다.

**빠르게 볼 수 있는 포인트**: LangChain을 활용한 RAG 실습.
...
- 읽을 포인트: 세부 흐름: LangChain 프롬프트 엔지니어링, LangChain 프롬프트 엔지니어링 > LLM - Prompt 체인 만들기 (LCEL 방식), LangChain 프롬프트 엔지니어링 > 메모리 활용
...
LangChain에서는 기존의 LLMChain.run() 방식 대신, LangChain Expression Language(LCEL)을 사용해 프롬프트 → LLM → 출력 파싱 과정을 하나의 체인으로 간단히 구성할 수 있습니다. summary_prompt / llm / StrOut...

#### LangChain 프롬프트 엔지니어링 > 메모리 활용

LLM은 이전 대화 내용(히스토리)을 함께 넣어주면 대화를 더 자연스럽게 이어갈 수 있습니다. LangChain 최신 버전에서는 history라는 자리에 누적된 대화 메시지를 자동으로 넣어주는 기능을 제공합니다. 그래서 같은 세션 ID로 체인을 호출하면, 이전 질문·답변들이 자동으...
...
1. LangChain을 활용한 RAG 실습: LangChain 프롬프트 엔지니어링, LangChain 프롬프트 엔지니어링 > LLM - Prompt 체인 만들기 (LCEL 방식)

## Code Highlights

### LangChain 프롬프트 엔지니어링

`LangChain 프롬프트 엔지니어링`는 이 노트에서 핵심 구현을 보여주는 코드 블록입니다. 코드 안에서는 모델 로드 흐름이 주석과 함께 드러납니다.
