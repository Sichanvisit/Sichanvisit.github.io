...
excerpt: "2 BERT LoRA에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LoRA - BERT 모델을 사용한... 순서로 핵심 장면을 먼저 훑고, LoRA - BERT 모델을 사용한 PEFT, 토크나이저 설정, PEFT - Lora 설정과 학습 같은 코..."
research_summary: "2 BERT LoRA에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LoRA - BERT 모델을 사용한... 순서로 핵심 장면을 먼저 훑고, LoRA - BERT 모델을 사용한 PEFT, 토크나이저 설정, PEFT - Lora 설정과 학습 같은 코드로 실제 구현을 이어서 확인할 수 있습니다. `md` 원본과 9개 코드 블록, 9개 실행 셀을 함께 남겨 구현 흐름을 다시 따라갈 수 있게 정리했습니다. 주요 스택은 huggingface_hub, transformers, datasets, peft입니다."
...
2 BERT LoRA에서 직접 따라간 구현 흐름과 코드 증거를 다시 볼 수 있게 정리한 LLM 학습 기록입니다. 본문은 LoRA - BERT 모델을 사용한... 순서로 핵심 장면을 먼저 훑고, LoRA - BERT 모델을 사용한 PEFT, 토크나이저 설정, PEFT - Lora 설정과 학습 같은 코드로 실제 구현을 이어서 확인할 수 있습니다. `md` 원본과 9개 코드 블록, 9개 실행 셀을 함께 남겨 구현 흐름을 다시 따라갈 수 있게 정리했습니다. 주요 스택은 huggingface_hub, transformers, datasets, peft입니다.

**빠르게 볼 수 있는 포인트**: LoRA - BERT 모델을 사용한 PEFT.
...
- 읽을 포인트: 세부 흐름: PEFT - Lora 설정과 학습, 모델 저장 (Save), 실제 테스트 (Inference)
...
1. LoRA - BERT 모델을 사용한 PEFT: PEFT - Lora 설정과 학습, 모델 저장 (Save)

## Code Highlights

### LoRA - BERT 모델을 사용한 PEFT

`LoRA - BERT 모델을 사용한 PEFT`는 이 노트에서 핵심 구현을 보여주는 코드 블록입니다. BERT 모델을 사용하여 PEFT(LoRA) 사용.
...
`모델 저장 (Save)`는 이 노트에서 핵심 구현을 보여주는 코드 블록입니다. 코드 안에서는 학습된 LoRA 모델(어댑터) 저장 흐름이 주석과 함께 드러납니다.
