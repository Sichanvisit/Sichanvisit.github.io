---
title: "2 Hotel Booking Demand"
date: 2026-03-08
research_tab: "ML"
research_kind: "Sprint Mission"
source_title: "[스프린트미션]2 Hotel Booking Demand - AI5 강사 답안"
source_path: "11_Machine_Learning/Code_Snippets/[스프린트미션]2 Hotel Booking Demand - AI5 강사 답안.md"
excerpt: "ML Sprint Mission 아카이브 엔트리입니다. 원본 실습 노트를 공개 research 섹션에서 구별하기 쉽게 정리한 카드입니다."
tags:
  - research-archive
  - imported-note
  - ml
  - sprint-mission
---

## Archive Note

이 글은 개인 실습 저장소에 있던 원본 노트를 `research` 컬렉션에서 구별해 보기 쉽게 정리한 아카이브 엔트리입니다.  
대표 항목은 이후 별도 케이스 스터디로 확장하고, 현재 단계에서는 전체 실습 흐름을 빠르게 탐색할 수 있도록 메타데이터 중심으로 정리했습니다.

| Item | Value |
|------|-------|
| Track | ML |
| Type | Sprint Mission |
| Source Title | `2 Hotel Booking Demand` |
| Source Path | `11_Machine_Learning/Code_Snippets/[스프린트미션]2 Hotel Booking Demand - AI5 강사 답안.md` |

## Source Glimpse

> 🏨 데이터 설명 / 2015.07.01부터 2017.08.31까지의 Resort Hotel과 City Hotel의 예약 데이터

## Notes

- 원본 파일은 수업 실습, 스프린트 미션, 강사 공유, 샘플 코드 중 하나로 분류했습니다.
- 현재 공개 블로그에서는 구분과 탐색을 우선하고, 의미 있는 항목부터 순차적으로 본문을 더 다듬을 예정입니다.
- 같은 탭 안에서도 `type` 배지로 미션과 실습을 바로 구별할 수 있게 구성했습니다.
