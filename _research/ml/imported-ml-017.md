...
excerpt: "코딩실습16 11.차원축소(PCA)의 원본 노트 흐름과 핵심 코드를 다시 따라갈 수 있게 정리한 ML 학습 기록입니다"
research_summary: "코딩실습16 11.차원축소(PCA)의 원본 노트 흐름과 핵심 코드를 다시 따라갈 수 있게 정리한 ML 학습 기록입니다. 본문은 차원 축소, 전처리와 입력 정리 순서로 큰 장을 먼저 훑고, 데이터 설명, StandardScaler 스케일링 같은 코드로 실제 구현을 이어서 확인할 수 있습니다. `ipynb/md` 원본과 11개 코드 블록, 10개 실행 셀을 함께 남겨 구현 흐름을 다시 따라갈 수 있게 정리했습니다. 주요 스택은 sklearn, matplotlib입니다."
...
| 항목 | 내용 |
|------|------|
| 문제 설정 | 64차원의 손글씨 데이터 사이킷런: https://scikit-learn.org/stable/modules/generated/sklearn.datasets.load_digits.html |
| 원본 구조 | 데이터 설명 |
| 데이터 맥락 | 64차원의 손글씨 데이터 사이킷런: https://scikit-learn.org/stable/modules/generated/sklearn.datasets.load_digits.html |
| 핵심 주제 | 차원 축소 · 전처리와 입력 정리 |
